...
public final class YoutubeRepository {
    ...
    public enum SearchSource { MUSIC_FIRST, YOUTUBE_MUSIC, YOUTUBE }
    ...
    public List<SearchResult> search(String query) throws Exception {
        ...
        if ("youtube_music".equals(source)) raw = searchYoutubeMusicSongs(normalized, 40);
    ...
    private static List<SearchResult> applyFilters(List<SearchResult> input, String query, SharedPreferences settings, int limit) {
        ...
        for (SearchResult item : input) {
            ...
            if (officialOnly && !isOfficialCandidate(title, channel)) continue;
    ...
    private File downloadAudioInternal(SearchResult item, ProgressListener listener) throws Exception {
        ...
        if (existing != null) { downloadSubtitlesBestEffort(item, dir); return existing; }
    ...
    private static boolean hasSubtitle(File dir, String id) { File[] files = dir.listFiles((d, name) -> { String n = name.toLowerCase(Locale.ROOT); return name.startsWith(id + ".") && (n.endsWith(".vtt") || n.endsWith(".srt")); }); return files != null && files.length > 0; }
