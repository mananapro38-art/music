...
public final class MusicWidgetProvider extends AppWidgetProvider {
    ...
    private static void updateWidgets(Context context, AppWidgetManager manager, int[] ids) {
        ...
        for (int id : ids) {
            manager.updateAppWidget(id, buildViews(context, title, artist, playing, null));
            if (artwork != null && !artwork.isEmpty()) {
                final int widgetId = id;
                IO.execute(() -> {
                    Bitmap bitmap = ArtworkLoader.loadBitmapBlocking(context, artwork);
                    if (bitmap != null) {
                        manager.updateAppWidget(widgetId, buildViews(context, title, artist, playing, bitmap));
                    }
                });
            }
        }
    ...
    private static RemoteViews buildViews(Context context, String title, String artist, boolean playing, Bitmap artwork) {
        ...
        if (artwork != null) views.setImageViewBitmap(R.id.widgetArtwork, artwork);
        else views.setImageViewResource(R.id.widgetArtwork, R.drawable.ic_music_note);
